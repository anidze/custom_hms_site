...
import { ChevronDown, X, Plus, Trash2, User, Calendar, BedDouble, CreditCard, MessageSquare, MapPin, Users } from "lucide-react";
...
export default function NewBookingPage() {
  ...
  const availableCities = useMemo(() => {
    ...
    if (!found) return [];
    return [...new Set((City.getCitiesOfCountry(found.isoCode) ?? []).map((c) => c.name))].sort();
  ...
  function addGuest() { setGuests((g) => [...g, { firstName: "", lastName: "", gender: "", age: "" }]); }
  function updateGuest(idx: number, field: keyof GuestRow, value: string) {
    setGuests((g) => g.map((row, i) => (i === idx ? { ...row, [field]: value } : row)));
  }
  function removeGuest(idx: number) { setGuests((g) => g.filter((_, i) => i !== idx)); }
